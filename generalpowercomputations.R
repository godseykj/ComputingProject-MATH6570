...
for (a in 1:15){
  ...
  for (i in 1:50000){
    ...
    # KS[i] <- ks.test(data, pnorm(0,1), alternative="two.sided", exact=TRUE)$statistic
...
Critvaluematrix <- cbind(ssizes, SWcrit, LLcrit, ADcrit, CVMcrit, CSQcrit)
...
for (a in 1:15){
  ...
  for(i in 1:10000) { 
    ...
    # KSd <- ks.test(data, pnorm(0,1), alternative="two.sided", exact=TRUE)$statistic
    ...
    # ifelse(KSd >= KScrit[a], testSW[i] <- 1, testSW[i] <- 0)
...
Unifpowermatrix <- cbind(ssizes, powerSW, powerLL, powerAD, powerCVM, powerCSQ)

# We can use knitr to output tables as needed as well
